...
public class CollectionToGoodsDAOImpl implements CollectionToGoodsDAO {
	/**
	 * cbid number(20) primary key, --编号-- jid number(20), --用户id-- gid
	 * number(20) primary key, --编号-- bid number(20), --企业用户id-- pid number(20),
	 * --商品id-- bnumber number, --商品库存数量-- pri number(20), --商品价格-- sale_val
	 * number, --销量-- col_val number, --收藏数量-- uptime varchar2(20), --上架时间--
	 * introduction varchar2(200), --商品简介-- detailname varchar2(50) --详细名称-- pid
	 * number(20) primary key, --编号-- pname varchar2(20), --商品名-- p_isVal number
	 * (2) --是否删除--
	 */
	@Override
	public List<CollectionToGoods> queryCollectionToGoodsByJid(int jid)
		...
		try {
			...
			while (rs.next()) {
				...
				Juser juser = new Juser(jid1, jname, jpwd, jemail, jtel,
						jgender, jbir, juserisval, jpic);
				Country country = new Country(countryid, country1, cpic);
				Buser buser = new Buser(bid, bname, blogo, isval, bpwd,
						bintroduction, bpic, bmpic, col_val, country);
				Product pro = new Product(pid, pname, p_isVal);
				Goods goods = new Goods(gid, buser, pro, bnumber, pri,
						sale_val, col_val1, uptime, introduction, detailname);
				CollectionToGoods ctb = new CollectionToGoods(cbid, juser,
						goods);
				list.add(ctb);
			}
			return list;
	 ...
	 * @throws DAOException
	 */
	public int queryJidByCbid(int cbid) throws DAOException {
		String sql = "SELECT JU.JID " + "FROM COLLECTIONTOGOODS COL,JUSER JU"
				+ " WHERE COL.JID=JU.JID AND COL.CBID=?";
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
